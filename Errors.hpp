...
class ComponentNotAttached : public std::exception
{
  public:
    ComponentNotAttached(std::size_t entity, const std::string &comp_name)
        : message("Entity id " + std::to_string(entity) + " doesn't have component '" + comp_name +
                  "' attached to it!")
    {
    }
...
class ComponentAlreadyAttached : public std::exception
{
  public:
    ComponentAlreadyAttached(std::size_t entity, const std::string &comp_name)
        : message("Entity id " + std::to_string(entity) + " already have component '" + comp_name +
                  "' attached to it!")
    {
    }
...
class TooManyComponentTypes : public std::exception
{
  public:
    TooManyComponentTypes(std::size_t max_types)
        : message("Cannot register more than " + std::to_string(max_types) +
                  " component types: the entity signature mask is out of bits!")
    {
    }
